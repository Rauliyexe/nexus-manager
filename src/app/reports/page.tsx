...
import { AudioReportStudio } from '@/components/audio/AudioReportStudio';

export default function ReportsPage() {
 const { areas, weeklyReports, financialMetrics } = useNexus();
 const [reportPeriod, setReportPeriod] = useState<'DAILY' | 'WEEKLY' | 'FORTNIGHTLY' | 'MONTHLY' | 'AUDIO_AI'>('AUDIO_AI');
 ...
 Consolidado diário, semanal, quinzenal, DRE e inteligência de processamento de áudio por voz
 </p>
 </div>
 ...
 <button
 onClick={() => setReportPeriod('AUDIO_AI')}
 className={`px-4 py-2 rounded-xl font-bold transition-all cursor-pointer flex items-center space-x-1.5 whitespace-nowrap ${
 reportPeriod === 'AUDIO_AI'
 ? 'bg-[#1B3026] text-white shadow-sm'
 : 'text-[#5C6E62] dark:text-slate-400 hover:text-[#1A281E] dark:hover:text-slate-200'
 }`}
 >
 <Mic className="w-3.5 h-3.5 text-emerald-400" />
 <span> Áudio → Relatório IA</span>
 <span className="px-1.5 py-0.2 rounded-full bg-emerald-500/20 text-emerald-300 text-[9px] uppercase font-mono">
 WOW
 </span>
 </button>
 ...
 {/* 0. ÁUDIO → RELATÓRIO ESTRUTURADO IA (WOW MOMENT) */}
 {reportPeriod === 'AUDIO_AI' && (
 <AudioReportStudio />
 )}
